...
- `--brief` (on `context` / `thread`) returns the decision layer only; `--navigate` returns lean navigation on the default packing budget; `--short` remains the legacy small-budget card mode.
