...
- `search` is always local discovery (accepts `--local` for symmetry), includes a permalink per candidate, defaults to the top 10 ranked candidates (`--limit <n>` overrides), and reports search coverage; use `context` before relying on a result.
...
3. Read `evidence` first; execute only `priority: "recommended"` from `next`. Do not invent optional sync/inspect follow-ups when absent. Use `brief` / `purposeHints` on primary then secondaries; skip `presentation: "announce"` and `surroundRelevance: "low"` unless needed.
4. Use `thread --full --agent` only for a selected incomplete thread; for `inspect_dropped`, copy the provided `thread … --agent` argv. When `filesPresent` or `files[]` is set, download with `file <id> --agent` or `files --out-dir` and Read before UI/screenshot-dependent claims. Do not re-`context` related tickets marked `alreadyInPacket`.
