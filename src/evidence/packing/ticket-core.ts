...
import { renderedPostUnits } from "./budget.ts";
import type { EvidencePost, PackTimelineItem } from "./types.ts";

export const DEFAULT_NEIGHBORHOOD_RADIUS = DEFAULT_MATCH_RADIUS;

export const DEFAULT_TICKET_NEIGHBORHOOD_RADIUS = DEFAULT_TICKET_RADIUS;

/** High-priority tail posts before gap-fill; remainder may fill later. */
export const LATEST_PRIORITY_COUNT = 3;

export const SHORT_LATEST_PRIORITY_COUNT = 2;

export const STRUCTURAL_LONG_MESSAGE_UNITS = 400;

export const DENSE_WINDOW_MS = 60 * 60 * 1000;

export const DENSE_WINDOW_MIN_POSTS = 3;
 ...
 */
export function selectContiguousTicketCore(
...
export function clusterMergeIds(
	chronological: readonly EvidencePost[],
 ...
 */
export function fillLargestInternalGaps(
	chronological: readonly EvidencePost[],
	byId: ReadonlyMap<string, EvidencePost>,
	selected: Set<string>,
	used: number,
...
export function largestInternalGapIds(
	chronological: readonly EvidencePost[],
...
export function edgeInwardOrder(ids: readonly string[]): string[] {
...
export function isStructuralPost(
	post: EvidencePost,
	options: { short?: boolean } = {},
): boolean {
...
export function isFileOrFencePost(post: EvidencePost | undefined): boolean {
...
export function densestWindowIds(
	chronological: readonly EvidencePost[],
): string[] {
